...
class Vocab(object):
    ...
    def fromVocabualry(self, voc, dim=100):
        ...
        scale = 0.05
        padding_vecs = np.zeros((1, self.word_dim), dtype=np.float32)
        normal_vecs = np.array(np.random.uniform(low=-scale, high=scale, size=shape), dtype=np.float32)
        self.word_vecs = np.concatenate((padding_vecs,normal_vecs,), axis=0)
...
#     print(vocab.word_dim)
    ...
    inpath = "/u/zhigwang/zhigwang1/sentence_generation/mscoco/data/wordvec.txt"
    print('Loading')
    word_vocab = Vocab(inpath, fileformat='txt3')
    print('word_vocab: {}'.format(word_vocab.word_vecs.shape))

    print('Dumping out')
    word_vocab.dump_to_txt2(inpath + ".with_idx")
    print('DONE!')
