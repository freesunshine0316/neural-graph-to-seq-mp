...
def split_heads(inputs, num_heads):
  ...
  assert len(static_shape) >= 3
  depth = static_shape[-1]
  outputs = tf.reshape(
          inputs, static_shape[:-1] + [num_heads, depth // num_heads])
  modified_shape = [static_shape[0], num_heads] + static_shape[1:-1] + [depth // num_heads]
  outputs = tf.transpose(outputs, perm=modified_shape)
  return outputs
...
def combine_heads(inputs):
  ...
  modified_shape = [static_shape[0]] + static_shape[2:-1] + [num_heads, depth]
  outputs = tf.transpose(inputs, perm=modified_shape)
  outputs = tf.reshape(outputs, modified_shape[:-2] + [depth * num_heads])
  return outputs
